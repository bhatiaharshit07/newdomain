...
<Mermaid
  chart={`graph LR
Cameras[Camera + Edge Sources] --> Edge[Edge Inference]
Edge --> Events[Event Normalization]
Events --> Rules[Rules + Context]
Rules --> Stream[Event Stream]
Stream --> Store[Event Store]
Store --> Analytics[Analytics Layer]
Analytics --> Alerts[Alerts + Workflows]
Analytics --> Dashboards[Decision Dashboards]
Dashboards --> Feedback[Operator Feedback]
Feedback --> Quality[Model + Rule Quality Loop]`}
/>
