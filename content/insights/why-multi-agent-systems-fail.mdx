...
<Mermaid chart="graph LR; Goal[User Goal] --> Route[Route]; Route --> Plan[Plan]; Plan --> Policy[Policy Check]; Policy --> Execute[Execute Tool]; Execute --> Evaluate[Evaluate Step]; Evaluate --> Approve[Approve or Escalate]; Approve --> Respond[Respond]; Respond --> Trace[Trace + Learn]" />
