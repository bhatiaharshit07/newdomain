...
<Mermaid chart="graph LR; Intent[Intent] --> Plan[Plan]; Plan --> Tools[Tools]; Tools --> Memory[Memory]; Plan --> Policy[Policy]; Policy --> Approval[Human Approval]; Tools --> Observe[Observability]; Observe --> Eval[Evaluation]; Eval --> Improve[Continuous Improvement]" />
